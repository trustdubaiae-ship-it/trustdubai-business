-- ============================================================
-- Marketplace upgrade — richer posts + trust surfaced.
--   • posts get scope-of-work items, project type, urgency
--   • the feed returns the POSTER company's trust (badge/score/rating) so a
--     replier sees who posted; the poster in turn joins each replier's company.
-- Run in Supabase → SQL Editor. Safe to re-run.
-- ============================================================

alter table public.qv_subcontract_projects add column if not exists scope        text[] not null default '{}';
alter table public.qv_subcontract_projects add column if not exists project_type text;
alter table public.qv_subcontract_projects add column if not exists urgency      text;

-- Feed now also returns scope/type/urgency + the poster company's trust fields.
-- (Drop+recreate: return type changed.)
