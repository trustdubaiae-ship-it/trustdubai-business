...
export default function LeadsPage() {
  ...
  async function fetchAll() {
    ...
    setForms(formsData || [])
    const active = formsData?.find(f => f.is_active)
    if (active) setActiveFormId(active.id)
  ...
  async function createForm() {
    ...
    const { data, error } = await supabase
      .from('lead_forms')
      .insert({ company_id: company.id, title: newFormTitle, is_active: forms.length === 0 })
      .select().single()
  ...
  async function saveForm() {
    ...
    // Delete existing questions aur re-insert
    ...
    if (toInsert.length > 0) {
      await supabase.from('lead_form_questions').insert(toInsert)
    }
  ...
  function updateOption(qId, idx, value) {
    setQuestions(prev => prev.map(q => q.id === qId
      ? { ...q, options: q.options.map((o, i) => i === idx ? value : o) } : q))
  }

  function removeOption(qId, idx) {
    setQuestions(prev => prev.map(q => q.id === qId
      ? { ...q, options: q.options.filter((_, i) => i !== idx) } : q))
  }
    ...
    <div className="page-content animate-in">
      <div style={{ marginBottom: 24, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          ...
          <p className="text-secondary" style={{ fontSize: 14 }}>Create multiple forms — set one as active on your public profile</p>
        </div>
        ...
        <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 'var(--radius)', padding: 20, marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <input
              ...
              placeholder="Form title e.g. Interior Design Inquiry"
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {forms.map(form => (
                <div key={form.id} className="card" style={{
                  borderColor: form.is_active ? '#03C1F5' : 'var(--card-border)',
                  borderWidth: form.is_active ? 2 : 1
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                        ...
                        {form.is_active && (
                          <span style={{ background: '#e0f9ff', color: '#03C1F5', fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 99, border: '1px solid #03C1F5' }}>
                            LIVE
                          </span>
                        )}
                      </div>
                      <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                        Created {new Date(form.created_at).toLocaleDateString('en-AE')}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <button
                        ...
                        style={{ padding: '6px 10px', background: '#fef2f2', color: '#ef4444', border: 'none', borderRadius: 6, cursor: 'pointer' }}
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
          ...
          <div>
            ...
            <div className="card">
              ...
              <div style={{ marginTop: 16, display: 'flex', gap: 10 }}>
                <button className="btn btn-primary" onClick={saveForm} disabled={saving} style={{ flex: 1, justifyContent: 'center' }}>
                  {saving ? 'Saving...' : 'Save Form'}
                </button>
                <button className="btn btn-secondary" onClick={() => setTab('forms')}>
                  Back
                </button>
              </div>
            </div>
