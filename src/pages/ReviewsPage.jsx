...
import { MessageCircle, Send, ChevronDown, ChevronUp, Flag, Clock } from 'lucide-react'
  ...
  return (
    <div className="page-content animate-in">
      <div style={{ marginBottom: 24 }}>
        <h1 className="font-syne fw-700" style={{ fontSize: 24, marginBottom: 4 }}>Reviews</h1>
        <p className="text-secondary" style={{ fontSize: 14 }}>Respond to customer reviews and manage your reputation</p>
      </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
          ...
          <div style={{ textAlign: 'center', flexShrink: 0 }}>
            <div style={{
              ...
              fontSize: 52, color: 'var(--text-primary)', lineHeight: 1
            }}>{avgRating}</div>
          ...
          <div style={{ flex: 1, borderLeft: '1px solid var(--card-border)', paddingLeft: 32 }}>
            {ratingCounts.map(({ star, count }) => (
              <div key={star} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                ...
                <div style={{
                  flex: 1, height: 6, background: 'var(--bg)', borderRadius: 99, overflow: 'hidden'
                }}>
                  <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, flexShrink: 0 }}>
            {[
              ...
              { label: 'Pending Reply', value: reviews.filter(r => !r.owner_reply).length, color: 'var(--amber)' },
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {!review.owner_reply && (
                      <span className="badge badge-gold" style={{ fontSize: 10 }}>Needs Reply</span>
                    )}
                  </div>
                ...
                <button className="btn btn-ghost btn-sm" title="Report review">
                  <Flag size={13} />
                </button>
              </div>
                ...
                {replyingTo === review.id ? (
                  <div style={{ flex: 1 }}>
                    <textarea
                      ...
                      style={{ minHeight: 80, fontSize: 13.5, marginBottom: 8 }}
                      autoFocus
                    />
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button
                      ...
                      >
                        {sendingReply ? <><div className="spinner" style={{ width: 12, height: 12 }} />Posting...</> : <><Send size={13} />Post Reply</>}
                      </button>
                      <button className="btn btn-secondary btn-sm" onClick={() => { setReplyingTo(null); setReplyText('') }}>
                        Cancel
                      </button>
                    </div>
                  </div>
