...
export default function Quotations({ subRoute = '', setSubRoute, startAi = false }) {
  ...
  function addFromLib(lib, trade='') {
    ...
    // allow the same item more than once (e.g. same work for a different room)
    const u = mapUnit(lib.unit)
  ...
  function buildQuoteHTML(q, voMeta) {
    ...
    if (canPremium) {
      ...
      // Room above the signing line. The artwork is 52px tall but the default
      // gap is only 24px, so without this the signature printed straight through
      // the "For <company>" line. Applied to BOTH columns so the two signing
