...
function getExpiryInfo(expiresAt) {
  ...
  if (diff < 0)   return { label:'Plan Expired!',     color:'#ef4444', bg:'rgba(239,68,68,0.12)', border:'rgba(239,68,68,0.4)', days:diff, expired:true,  urgent:true  }
  if (diff <= 7)  return { label:`${diff} days left`, color:'#ef4444', bg:'rgba(239,68,68,0.12)', border:'rgba(239,68,68,0.4)', days:diff, expired:false, urgent:true  }
  if (diff <= 30) return { label:`${diff} days left`, color:'#f59e0b', bg:'rgba(245,158,11,0.12)', border:'rgba(245,158,11,0.4)', days:diff, expired:false, urgent:false }
  return            { label:`${diff} days left`,      color:'#10b981', bg:'rgba(16,185,129,0.12)', border:'rgba(16,185,129,0.4)', days:diff, expired:false, urgent:false }
}
...
function GlowCard({ glow, children, style, onClick, onMouseEnter, onMouseLeave }) {
  return (
    <div onClick={onClick} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}
      style={{ position:'relative', overflow:'hidden', background:'var(--card)', border:'0.5px solid var(--border)', borderRadius:14, padding:16, ...style }}>
...
function Sparkline({ data, color, height=30 }) {
  if (!data||data.length<2) return (
    <svg width="84" height={height}><line x1="0" y1={height/2} x2="84" y2={height/2} stroke={color} strokeWidth="1.5" opacity="0.3" strokeDasharray="3,2"/></svg>
  )
  const max=Math.max(...data), min=Math.min(...data), range=max-min||1
  const pts=data.map((v,i)=>`${(i/(data.length-1))*84},${height-((v-min)/range)*(height-4)-2}`).join(' ')
  return (
    <svg width="84" height={height} style={{ overflow:'visible' }}>
      <polyline points={pts} fill="none" stroke={color} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" opacity="0.9"/>
      <circle cx={84} cy={height-((data[data.length-1]-min)/range)*(height-4)-2} r="2.6" fill={color}/>
    </svg>
  )
}
...
function TrustGauge({ score }) {
  ...
  return (
    <svg width="104" height="104" viewBox="0 0 100 100">
      ...
      <text x={cx} y={cy-3} textAnchor="middle" fill={color} fontSize="19" fontWeight="700">{score}</text>
      <text x={cx} y={cy+12} textAnchor="middle" fill="var(--text3)" fontSize="8">Trust Score</text>
    </svg>
  )
}
    ...
    <svg width="110" height="110" viewBox="0 0 110 110">
      ...
      {total>0 && segs.map(([c,v],i)=>{
        ...
        const el=<circle key={i} cx={cx} cy={cy} r={r} fill="none" stroke={c} strokeWidth="11"
          strokeDasharray={`${len} ${circ-len}`} strokeDashoffset={-offset+circ*0.25} transform={`rotate(0 ${cx} ${cy})`} />
        offset+=len
        return el
      })}
...
function AIScoreRow({ label, score, color }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:9 }}>
      ...
      <div style={{ width:110, height:4, background:'var(--bg2)', borderRadius:99, overflow:'hidden' }}>
...
function ClockCard({ defaultName }) {
  ...
  const digitStyle = {
    ...
    fontSize:28, fontWeight:800, color:digitFg, fontVariantNumeric:'tabular-nums', minWidth:48, textAlign:'center', display:'inline-block', lineHeight:1,
  }
...
export default function DashboardPage({ onNavigate }) {
  ...
  async function fetchStats() {
    try {
      ...
      const repGrowth      = lastMonthCount===0 ? (newRev>0?'High':'Low') : newRev>lastMonthCount?'High' : newRev===lastMonthCount?'Stable':'Low'

      const satisfaction = avg>0 ? Math.round((parseFloat(avg)/5)*100) : 0
      const verified     = company.is_verified ? 1 : 0
      const trustScore   = Math.min(10, parseFloat(((verified*0.4)+(parseFloat(avg)/5*0.4)+Math.min(reviews.length/50,1)*0.2)*10).toFixed(1))
  ...
  const trustSuggestions = [
    { done:company?.is_verified,            label:'Get Trade License Verified', points:'+4.0 pts' },
    { done:parseFloat(stats.avgRating)>=4,  label:'Maintain 4+ star rating',    points:'+4.0 pts' },
    { done:stats.reviews>=10,               label:'Get 10+ customer reviews',   points:'+2.0 pts' },
    { done:!!company?.logo_url,             label:'Upload company logo',        points:'Profile boost' },
    ...
    <div className="page-content animate-in" style={{ color:'var(--text)' }}>
        ...
        {statCards.map((card,i)=>(
          <GlowCard key={i} glow={card.glow}
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:9 }}>
              <div style={{ width:30, height:30, borderRadius:8, background:card.color+'22', display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'2fr 1.2fr 1fr 1fr 1fr', gap:12, marginBottom:14 }}>
        <GlowCard glow="rgba(16,185,129,0.1)">
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
            <div>
              <div style={sectionTitle}>Customer Sentiment Analytics</div>
          ...
          <div style={{ position:'relative', height:100 }}>
            <div style={{ position:'absolute', left:24, right:0, top:0, bottom:16 }}>
              <svg width="100%" height="100%" viewBox="0 0 400 84" preserveAspectRatio="none">
                {[0,42,84].map(y=><line key={y} x1="0" y1={y} x2="400" y2={y} stroke="var(--border)" strokeWidth="0.5"/>)}
                <path d="M0,70 40,62 80,55 120,45 160,38 200,30 240,22 280,15 320,10 360,7 400,5 L400,84 L0,84 Z" fill="rgba(16,185,129,0.12)"/>
            ...
            <div style={{ position:'absolute', left:0, top:0, bottom:16, display:'flex', flexDirection:'column', justifyContent:'space-between', width:22 }}>
              {['100','50','0'].map(l=><span key={l} style={{ fontSize:7, color:'var(--text3)' }}>{l}</span>)}
            </div>
            <div style={{ position:'absolute', left:24, right:0, bottom:0, display:'flex', justifyContent:'space-between' }}>
              {['Jan','Mar','May','Jul','Sep','Nov'].map(m=><span key={m} style={{ fontSize:7, color:'var(--text3)' }}>{m}</span>)}
            </div>
          </div>
        </GlowCard>
        ...
        <GlowCard glow="rgba(139,92,246,0.14)">
          <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:10 }}>
            ...
            <div style={sectionTitle}>AI Business Summary</div>
          </div>
          {[
        ...
        <GlowCard glow="rgba(16,185,129,0.12)">
          ...
          <div style={{ display:'flex', justifyContent:'center', marginBottom:8 }}>
            <TrustGauge score={parseFloat(stats.trustScore)||0}/>
          </div>
          <div style={{ textAlign:'center', marginBottom:10 }}>
            <span style={{ background:'var(--green-bg)', color:'#10b981', fontSize:9, fontWeight:700, padding:'3px 9px', borderRadius:99, border:'0.5px solid rgba(16,185,129,0.4)' }}>
        ...
        <GlowCard glow="rgba(232,184,75,0.12)">
          ...
          {[['Service Quality','82%'],['Professionalism','55%'],['Value for Money','40%'],['Timeliness','12%']].map(([l,v])=>(
            <div key={l} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:5 }}>
              <span style={{ fontSize:9, color:'var(--text2)', flex:1 }}>{l}</span>
              <div style={{ width:54, height:3, background:'var(--bg2)', borderRadius:99, overflow:'hidden' }}>
          ...
          <div style={{ fontSize:9, fontWeight:700, color:'var(--text2)', margin:'8px 0 6px' }}>What People Dislike</div>
            ...
            <div key={r.id} style={{ padding:'8px 0', borderBottom:i<recentReviews.length-1?'0.5px solid var(--border)':'none' }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:4 }}>
                <div style={{ display:'flex', alignItems:'center', gap:7 }}>
                  ...
                  <span style={{ fontSize:10, fontWeight:600, color:'var(--text)' }}>{r.reviewer_name||'Anonymous'}</span>
                </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <GlowCard glow={company?.is_verified?'rgba(16,185,129,0.14)':'rgba(239,68,68,0.14)'} style={{ flex:1 }}>
            ...
            <div style={{ textAlign:'center', padding:'8px 0' }}>
              ...
              <div style={{ fontSize:9, color:'var(--text3)' }}>Certification Level</div>
            </div>
