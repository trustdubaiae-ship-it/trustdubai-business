...
function Sparkline({ data, color, height=32 }) {
  ...
  const max=Math.max(...data), min=Math.min(...data), range=max-min||1, w=80
...
export default function DashboardPage({ onNavigate }) {
  ...
  const [stats,         setStats]         = useState({ views:0, reviews:0, avgRating:0, portfolio:0, newReviews:0, satisfaction:88, reputationGrowth:'High' })
  const [recentReviews, setRecentReviews] = useState([])
  const [memberCount,   setMemberCount]   = useState(0)
  const [reviewDist,    setReviewDist]    = useState({ 5:0, 4:0, 3:0, 2:0, 1:0 })
  const [loading,       setLoading]       = useState(true)

  // Stable clock via refs
  ...
  async function fetchStats() {
    try {
      ...
      setReviewDist(dist)
      setStats({ views:company.profile_views||0, reviews:reviews.length, avgRating:avg, portfolio:portfolioRes.data?.length||0, newReviews:newRev, satisfaction:avg>0?Math.round(avg/5*100):0, reputationGrowth: avg>=4.5?'High':avg>=3.5?'Medium':'Low' })
  ...
  const trustScore  = stats.avgRating>0 ? Math.min(10,(parseFloat(stats.avgRating)*2)).toFixed(1) : '0.0'
    ...
    <div className="page-content animate-in" style={{ color:C.text, background: isDark?'#0f0e1a':'var(--bg)' }}>
      ...
      {expiryInfo?.urgent && plan!=='free' && (
        <div style={{ background:expiryInfo.bg, border:`0.5px solid ${expiryInfo.border}`, borderRadius:10, padding:'12px 18px', display:'flex', alignItems:'center', gap:12, marginBottom:16 }}>
          ...
          <button className="btn btn-primary btn-sm" onClick={()=>window.open(`https://wa.me/971503856786?text=Hi, I need to renew my TrustDubai ${plan} plan`,'_blank')}>
            Renew Now
          </button>
        </div>
      )}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:10, marginBottom:16 }}>
        {[
          { label:'Trust Score',        value:trustScore,              icon:'ti-shield-check', color:'#10b981', trend:[5,5.5,6,6.2,7,7.5,8,8.2,9,9.1,9.2,parseFloat(trustScore)||0], change:'+6.5% Change', isScore:true },
          { label:'Total Reviews',      value:stats.reviews,           icon:'ti-message-circle',color:'#e8b84b', trend:[2,5,8,12,18,25,30,42,55,70,85,stats.reviews], change:'+3.1% Change' },
          { label:'New Reviews (30D)',   value:stats.newReviews,        icon:'ti-star',         color:'#3b82f6', trend:[0,1,2,3,5,4,6,8,7,9,10,stats.newReviews],    change:'+12% Change' },
          { label:'Average Rating',     value:stats.avgRating||'0.0',  icon:'ti-star-filled',  color:'#f59e0b', trend:[3,3.2,3.5,3.8,4,4.1,4.3,4.5,4.6,4.7,4.8,parseFloat(stats.avgRating)||0], change:'+0.1 Change', isRating:true },
          ...
          { label:'Reputation Growth',  value:stats.reputationGrowth,  icon:'ti-trending-up',  color:'#10b981', trend:[20,30,35,45,40,55,60,65,70,75,80,stats.reputationGrowth==='High'?90:stats.reputationGrowth==='Medium'?60:30], change:'+7.4% Change', isStr:true },
        ].map((card,i) => (
          <div key={i} style={{ ...cardS, cursor:'pointer', transition:'all 0.15s', position:'relative', overflow:'hidden' }}
            onMouseEnter={e=>{ e.currentTarget.style.borderColor=card.color+'55'; e.currentTarget.style.transform='translateY(-1px)' }}
          ...
          >
            ...
            <div style={{ fontSize:card.isStr?18:20, fontWeight:700, color:C.text, lineHeight:1, marginBottom:6 }}>
              {loading ? '—' : card.isRating ? `${card.value}★` : card.value}
            </div>
      ...
      {/* MIDDLE ROW: Sentiment Chart + AI Summary + Review Distribution + Trust Gauge + Review Highlights */}
      <div style={{ display:'grid', gridTemplateColumns:'2fr 1.2fr 1fr 1fr 1fr', gap:12, marginBottom:14 }}>
        ...
        <div style={{ ...cardS, background: isDark?'rgba(139,92,246,0.1)':'#fafafa', border:`0.5px solid ${isDark?'rgba(139,92,246,0.2)':'#e2e8f0'}` }}>
          <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:10 }}>
            ...
            <i className="ti ti-settings" style={{ fontSize:12, color:C.text3, marginLeft:'auto' }}/>
          </div>
          {[
        ...
        <div style={cardS}>
          <div style={{ fontSize:11, fontWeight:700, color:C.text, textTransform:'uppercase', letterSpacing:'0.04em', marginBottom:10 }}>Trust Score Visualization</div>
          <div style={{ display:'flex', justifyContent:'center', marginBottom:8 }}>
            <TrustGauge score={parseFloat(trustScore)||0} isDark={isDark}/>
          </div>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', fontSize:9, color:C.text2, marginTop:6 }}>
            <span>Community Trust Level</span>
            <span style={{ color:'#10b981', fontWeight:600 }}>High</span>
          </div>
      ...
      {/* BOTTOM ROW: AI Insights + Profile Checklist + Recent Reviews + Business Perf + Verification + Premium */}
          ...
          {recentReviews.length===0 ? (
            ...
            <div key={r.id} style={{ padding:'8px 0', borderBottom:i<recentReviews.length-1?`0.5px solid ${C.border}`:'none' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:4 }}>
                ...
                <span style={{ fontSize:8, background:'rgba(16,185,129,0.1)', color:'#10b981', padding:'1px 6px', borderRadius:99, fontWeight:600 }}>98%</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          <div style={{ ...cardS, flex:1 }}>
            ...
            <div style={{ textAlign:'center', padding:'10px 0' }}>
              <div style={{ width:50, height:50, borderRadius:'50%', background:'rgba(16,185,129,0.1)', border:'2px solid #10b981', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 8px' }}>
                <i className="ti ti-shield-check" style={{ fontSize:24, color:'#10b981' }}/>
              </div>
              <div style={{ fontSize:11, fontWeight:700, color:'#10b981', marginBottom:2 }}>Verified Business Badge</div>
            ...
            <div style={{ height:4, background:C.bar, borderRadius:99, overflow:'hidden', margin:'6px 0' }}>
              <div style={{ width:'75%', height:'100%', background:'#10b981', borderRadius:99 }}/>
            </div>
          </div>
          ...
          <div style={{ ...cardS, background: isDark?'rgba(139,92,246,0.1)':'#faf5ff', border:`0.5px solid ${isDark?'rgba(139,92,246,0.2)':'#e9d5ff'}` }}>
            ...
            {[
              { icon:'ti-star', label:'Featured Listings' },
              { icon:'ti-ad-2', label:'Sponsored Placement' },
              { icon:'ti-chart-bar', label:'Advanced Analytics' },
              { icon:'ti-robot', label:'AI Reputation Tools' },
            ].map(f => (
              <div key={f.label} style={{ display:'flex', alignItems:'center', gap:8, padding:'5px 0', borderBottom:`0.5px solid ${isDark?'rgba(139,92,246,0.1)':'#f3e8ff'}` }}>
      ...
      {/* BUSINESS PERFORMANCE + PLAN INFO */}
      <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr 1fr', gap:12, marginBottom:14 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:12 }}>
            ...
            <div>
              ...
              {expiryInfo && plan!=='free' ? (
                ...
                <div style={{ fontSize:10, color:C.text3, marginTop:2 }}>{plan==='free'?'Upgrade for more':'✓ Plan active'}</div>
              )}
            </div>
          </div>
        ...
        {/* Platinum CTA or Profile Completion */}
        {plan!=='platinum' ? (
          ...
          <div style={{ ...cardS, display:'flex', flexDirection:'column', gap:10 }}>
            ...
            {profilePct<100 && (
              <button className="btn btn-primary btn-sm" onClick={()=>onNavigate('profile')}>Complete Profile</button>
            )}
          </div>
        )}
